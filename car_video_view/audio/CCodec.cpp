...
#define MY_AUDIO_RESULT_BUF_LEN 4096
CCodec::CCodec()
{
    m_g726_decoder_ctx = nullptr;
    m_iResult.m_nOutBufLen = MY_AUDIO_RESULT_BUF_LEN;
    m_iResult.m_pOutBuf = (char*)new char[MY_AUDIO_RESULT_BUF_LEN];
    m_iResult.m_eType = AUDIO_CODING_TYPE::eUnSupport;

    m_resultEncode.m_nOutBufLen = MY_AUDIO_RESULT_BUF_LEN;
    m_resultEncode.m_pOutBuf = (char*)new char[MY_AUDIO_RESULT_BUF_LEN];
...
bool CCodec::HaveHaiSiHeader(const char* pData, size_t nDataLen)
{
    ...
    if (pData[0] != 0x00 || pData[1] != 0x01 || pData[3] != 0x00) {
        return false;
    }
...
{ //.............................g726part
    ...
    int n = decode(m_g726_decoder_ctx, cInBuf, nInBufLen, m_iResult.m_pOutBuf, MY_AUDIO_RESULT_BUF_LEN); // Decode
...
void CCodec::__EncodeG7262Pcm(const char* pInBuf, int nInBufLen)
{
    ...
    int n = encode(m_g726_coder_ctx, pInBuf, nInBufLen, p, MY_AUDIO_RESULT_BUF_LEN - 4);
    // 编码失败时，发送空数据包
    if (n <= 0) {
        m_resultEncode.m_nOutBufLen = 0;
    } else {
